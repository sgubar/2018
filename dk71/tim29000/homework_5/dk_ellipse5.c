...
float Ellipse_Area(Ellipse *eEllipse)
{
    ...
    if(eEllipse != NULL)
    {
        float aSide = (float)sqrt(pow(eEllipse->C->x - eEllipse->Centre->x, 2)+pow(eEllipse->C->y - eEllipse->Centre->y ,2));
        float bSide = (float)sqrt(pow(eEllipse->B->x - eEllipse->Centre->x, 2)+pow(eEllipse->B->y - eEllipse->Centre->y ,2));
        result = 3.14*aSide * bSide;
    }
...
void writePoint(FILE *eFile, Point *ePoint)
{
    ...
    fprintf(eFile, "{ %d ,  %d}", ePoint->x, ePoint->y);
}
