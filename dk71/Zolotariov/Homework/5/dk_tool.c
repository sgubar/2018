...
float GetArea(Trapeze* trapeze) {
	float a = sqrt(pow(trapeze->points[2]->x - trapeze->points[1]->x, 2) + pow(trapeze->points[2]->y - trapeze->points[1]->y, 2));
	float b = sqrt(pow(trapeze->points[3]->x - trapeze->points[0]->x, 2) + pow(trapeze->points[3]->y - trapeze->points[0]->y, 2));
	float c = sqrt(pow(trapeze->points[1]->x - trapeze->points[0]->x, 2) + pow(trapeze->points[1]->y - trapeze->points[0]->y, 2));
	float d = sqrt(pow(trapeze->points[3]->x - trapeze->points[2]->x, 2) + pow(trapeze->points[3]->y - trapeze->points[2]->y, 2));
	float h = sqrt((b - a + c + d)*(a - b + c + d)*(a - b + c - d)*(a - b - c + d)) / (2 * abs(b - a));
	return h*(a + b / 2);
}
...
void ShellSort(TrapezeArray* arr, int n)
{
	...
	for (i = n / 2; i > 0; i = i / 2)
	{
		for (j = i; j < n; j++)
		{
			for (k = j - i; k >= 0; k = k - i)
			{
				// If area at higher index is greater, then break the loop.
				if (GetArea(arr->array[k + i]) >= GetArea(arr->array[k]))
					break;
				// Switch the trapezes otherwise.
				else
				{
					temp = arr->array[k];
					arr->array[k] = arr->array[k + i];
					arr->array[k + i] = temp;
				}
			}
		}
...
int BinarySearñh(TrapezeArray* arr, Trapeze* trapeze, int area, int start, int end) {
	area = area ? area : 0;
	end = end ? end : sizeof(arr->array) / sizeof(arr->array[0]);
	start = start ? start : 0;
	int middle = (end - start) / 2;
	area = GetArea(trapeze);
	if (area == GetArea(arr->array[middle])) {
		return arr->array[middle];
	}
	else if (area < GetArea(arr->array[middle]))
	{
		return BinarySearñh(arr, trapeze, area, start, middle - 1);
	}
	else {
		return BinarySearñh(arr, trapeze, area, middle + 1, end);
	}
	return -1;
}
...
void PrintArr(TrapezeArray* arr) {
	int i;
	for (i = 0; i < sizeof(arr->array) / sizeof(arr->array[0]); i++) {
		print_Trapeze(arr->array[i]);
	}
}
