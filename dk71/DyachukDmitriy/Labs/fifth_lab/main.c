...
int main(void)
{
	...
	array_of_values = (float *)malloc(sizeof(float) * num_of_elements);
	printf("\n\n- Please enter an float value for the root of your tree ...\n\n- Your value: ");
	...
	do
	{
		...
		if(Node != NULL)
		{
			printf("\n\n- Value which you search: %f",Node->Value);
			if(Node->Parent)
				printf("\n\n- Previous value: %f", Node->Parent->Value);
			else
				printf("\n\n- Your value is a root of your binary tree ...");
			if(Node->Left_Child)
				printf("\n\n- Lowest value %f", Node->Left_Child->Value);
			if(Node->Right_Child)
				printf("\n\n- Highest value: %f", Node->Right_Child->Value);
		}else
		{
