...
prtriangle2d new_rtriangle2d_arr(ppoint2d* arr){
	prtriangle2d ret = malloc(sizeof(rtriangle2d));
	for (int i = 0; i < 3; ++i)
	{
...
invalid: 
	...
	prtriangle2d ret = new_rtriangle2d_coords(args[0][0], args[0][1], args[1][0], args[1][1], args[2][0], args[2][1]);
	if(validate(ret) == INVALID){
		free(ret);
...
double area(prtriangle2d in){
	...
	return (sqrt(p*(p-sidelen[0])*(p-sidelen[1])*(p-sidelen[2])));
}
