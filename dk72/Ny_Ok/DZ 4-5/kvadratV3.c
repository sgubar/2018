...
printf ("Координаты КВАДРАТА: A(%d,%d) - B(%d,%d) - C(%d,%d)- D(%d,%d)",
      aKvadrat->A->x, aKvadrat->A->y, aKvadrat->B->x, aKvadrat->B->y,
      aKvadrat->C->x, aKvadrat->C->y, aKvadrat->D->x, aKvadrat->D->y);
    }
}
/*-----------------------------------------------*/
...
Point *Result = (Point *)malloc(sizeof(Point));
    if (NULL != Result)/* если с выделением памяти всё в порядке*/
    {
     Result->x = pPoint->x;
     Result->y = pPoint->y;
    }
return Result;/*возвращается структура (указатель)*/
}
