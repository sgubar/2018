...
int partition(KvadratArray *anArray, int l, int r)
{
   int i, j;
   Kvadrat *t;
   int pivot = anArray->ploschaKvadrat[l];
   i = l; j = r+1;

   while( 1)
   {
   	do ++i; while( anArray->ploschaKvadrat[i] <= pivot && i <= r );
   	do --j; while( anArray->ploschaKvadrat[j] > pivot );
   	if( i >= j ) break;
...
int linijnyj_poshuk(KvadratArray *anArray, float key)
{
    for (int i = 0; i < anArray->count; i++)
    {
        if (anArray->ploschaKvadrat[i] == key)
        return i+1;
